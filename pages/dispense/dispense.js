...
const Dispense=({token, patient={patientNote:"", qoHand:0, patientName:"", medicationName:"", sampleId:0, disburementQuantity:0, patientMobileNo:""}, update=false})=>{
    const router = useRouter()
    const [patientsName, setpatientsName] = useState(patient.patientName||"")
    const [dosage, setdosage] = useState({label:patient.medicationName === "" ?"":patient.medicationName+" | "+patient.lot+" | "+formateIncomingDate(patient.expiryDate)||"", value:patient.sampleId||0})
    ...
    return(
        ...
        <div className={styles.add_samples_title}>{update?"Update dispense details":"Dispense Medication"}
            <hr/>
        </div>
        ...
        <input className={styles.add_samples_input} value={patientsName} onChange={(e)=> setpatientsName(e.target.value)}/>
