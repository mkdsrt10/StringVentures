    ...
    useEffect(async () => {
        ...
        if (data){
            const LData = (data||[]).map(d => {
                return {label:d.medicationName, value:d.ID}
            })
            setSamples(LData)
        }
    }, [token])
    ...
    async function addDetails() {
        if (name.length < 3) {
            ...
            const {data, error} = await createDisburement({
                token, sample: {
                    patientNote:patientNote, patientName:name, sampleId: mediactionname,
                    disburementQuantity:parseInt(quantity), patientMobileNo:phone
                }
            })
            if (error){
                if (error===true){
    ...
    return(
        <div className={styles.main_container}>
            <div className={styles.top_divs}>
                <span className={styles.span2}>Disbursement details</span>
            </div>
            <div className={styles.input_fields}>
                <div className={styles.items}>
                    <label className={styles.label}>Identify</label>
