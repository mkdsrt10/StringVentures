...
import {createNewSample, updateSample} from "../../function/samples";
...
const AddSamples=({token, onClose=async () => {}, sampleR=null, update=false})=>{
    ...
    async function addDetails() {
        ...
        } else if (expiryDate !== "" && expiryDate !== null && !/^[0-9]{2}\/[0-9]{2}\/[0-9]{2}$/.test(expiryDate)) {
            setError("Expiry date of medication not valid (format is MM/DD/YY) Keep empty if not visible."+expiryDate+"::")
        } else {
            setError("")
        ...
        if (selectedImage.length < 3) {
            ...
            const {data, error} = await updateSample({
                token, sample: {
                    ...
                    medicationName: medicationName, unitOfM:unitOfM.value, quantityPerU: quantityPerU,
                    lot:sampleR.lot,
                    expiryDate:(expiryDate !== "" && expiryDate !== "01/01/01" && expiryDate !== null) ? toAPIFormat(expiryDate) : sampleR.expiryDate, status:sampleR.status||(lot !== "" && lot != null && expiryDate !== "" && expiryDate !== "01/01/01" && expiryDate !== null && /^[0-9]{2}\/[0-9]{2}\/[0-9]{2}$/.test(expiryDate))?"Approved":"Pending",
                    receivedQuantity:parseInt(receivedQuantity), repName:repName,
    ...
    return(
        <div className={styles.add_samples_main_mobile}>
            <div className={styles.add_samples_title}>Add sample details<hr/></div>
            ...
            }
            {
                ...
                <div className={styles.add_samples_sub_row}>
                    <div className={styles.add_samples_sub_row_left}>
                        ...
                        <input className={styles.add_samples_input} value={lot} onChange={(e)=> setLot(e.target.value)}/>
                    </div>
                    <div className={styles.add_samples_sub_row_right}>
                        <span className={styles.add_samples_label}>Expiry Date (MM/DD/YY)</span>
                        <input className={styles.add_samples_input} value={(expiryDate)} onChange={(e)=> setExpiryDate(e.target.value)}/>
                    </div>
                </div>
            }
            <div className={styles.add_samples_sub_row}>
                ...
                <div className={styles.add_samples_sub_row_right}>
                    ...
                    <input className={styles.add_samples_input} value={quantityPerU} onChange={(e)=> setQuantityPerU(e.target.value)}/>
                </div>
            </div>
            <div className={styles.add_samples_sub_row}>
                <div className={styles.add_samples_sub_row_left}>
                    <span className={styles.add_samples_label}>Received Units</span>
                    <input className={styles.add_samples_input} value={receivedQuantity} onChange={(e)=> setreceivedQuantity(e.target.value)}/>
                </div>
                <div className={styles.add_samples_sub_row_right}>
                    <span className={styles.add_samples_label}>Rep name</span>
                    <input className={styles.add_samples_input} value={repName} onChange={(e)=> setrepName(e.target.value)}/>
                </div>
            </div>
            {
